...
/**
 * The payload's route is the backend's wire contract, and it still names `/pay`
 * from when Pay was a tab. Home is the money surface now and the Pay flow is a
 * pushed route, so the wire value is translated here rather than in the service
 * that defines the contract — the contract is not ours to change.
 */
function toAppRoute(route: NotificationRoute) {
  return route === '/pay' ? '/send-money' : route;
}
...
export function useNotificationTaps() {
  ...
  useEffect(() => {
    ...
    const open = (data: unknown) => router.navigate(toAppRoute(resolveNotificationRoute(data)));
